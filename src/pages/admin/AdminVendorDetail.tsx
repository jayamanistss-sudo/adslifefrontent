...
const STATUS: Record<string, { bg: string; text: string; dot: string }> = {
  ...
  fraud_review:   { bg: 'bg-red-50 dark:bg-red-950/30',        text: 'text-red-700 dark:text-red-300',         dot: 'bg-red-600'     },
...
export default function AdminVendorDetail() {
  ...
  const action = async (act: string, note?: string) => {
    setActing(true);
    try {
      const res = await api.put(endpoints.adminVendorAction(Number(id)), { action: act, ...(note ? { note } : {}) });
      const cascaded = res.data.data?.cascaded_offers;
      toast.success(cascaded ? `${res.data.message ?? 'Done'} — deactivated ${cascaded} offer(s)` : (res.data.message ?? 'Done'));
      load();
    } catch { toast.error('Action failed'); }
    finally { setActing(false); }
  };

  // Suspend/reject previously had no note field in the UI at all — the note
  // was collected only on the initial application review, then silently
  // discarded even there once it reached the backend.
  const actionWithNote = (act: string, confirmMsg: string) => {
    const note = window.prompt(confirmMsg) ?? undefined;
    if (note === undefined) return; // cancelled
    action(act, note || undefined);
  };
    ...
    <div className="pb-10 w-full">
        ...
        <div className="p-6">
          <div className="flex items-start gap-4 mb-5">
            ...
            <div className="flex flex-wrap gap-2 flex-shrink-0">
              ...
              {v.status === 'approved' && (
                <button disabled={acting} onClick={() => actionWithNote('suspend', `Suspend ${v.business_name}? This will also deactivate all of their live offers.\n\nReason (optional, shown to the vendor):`)}
              ...
              {(v.status === 'pending_review' || v.status === 'fraud_review') && (
                <button disabled={acting} onClick={() => actionWithNote('reject', `Reject ${v.business_name}?\n\nReason (optional, shown to the vendor):`)}
          ...
          {v.status === 'fraud_review' && (
            <div className="mb-4 px-4 py-2.5 rounded-xl bg-red-50 dark:bg-red-950/30 border border-red-100 dark:border-red-900/30 text-xs text-red-700 dark:text-red-300">
              Automatically held on approval — the fraud scorer flagged this vendor above the auto-reject threshold. See <button onClick={() => navigate('/admin/fraud')} className="underline font-semibold">Fraud Dashboard</button> for details, or Approve/Reject above.
            </div>
          )}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5 mb-5">
        ...
        <div className="xl:col-span-2 card p-0 overflow-hidden">
          ...
          {v.review_note && (v.status === 'suspended' || v.status === 'rejected') && (
            <div className="mx-5 mb-5 p-3 rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-100 dark:border-amber-900/30">
              <p className="text-[10px] text-[var(--text-muted)] uppercase tracking-wide mb-1">Reason for {v.status}</p>
              <p className="text-sm text-[var(--text)]">{v.review_note}</p>
            </div>
          )}
        </div>
